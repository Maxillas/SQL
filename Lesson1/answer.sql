...
средняя (EmployeeTerritories и OrderDetails) является связующей и хранит 
пары EmployeeID - TerritoryID и OrderID - ProductID 

Рефлексия:
Не учел "направление" связи, хотя в голове это держал, но в задании явно не указал 
стрелочкой направления. На заметку.