# Рефлексия:
1. Не учел, что при запрсое мы уже знаем об одной из БД (например dwarves из первого)
Из-за этого знания можно обойтись простым where, без join
2. Во второй задаче не вывел именно ВХОДЯЩИЕ материалы, по ошибке решил, что материалы все по умолчанию входящие
